    ...
    try {
      ...
      if (jsonRes['success'] ?? false) {
        if (bundle) {
          ...
          var dataList = jsonRes['data'] is List ? jsonRes['data'] : [jsonRes['data']];
  ...
  static Future<SingleCourseModel?> getSingleCourseData(int id, bool isBundle, {bool isPrivate = false}) async {
    try {
      ...
      if (jsonRes['success'] ?? false) {
        ...
        int? categoryId = isBundle ? null : jsonRes['data']['category_id'];

        // Log category ID if applicable, but don't set it for bundles
        if (categoryId != null && !isBundle) {
          await AppData.setCategoryId(categoryId); // This is only set if it's not a bundle
        }
